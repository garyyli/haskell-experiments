...
allIntegersExcludingFirst a b = if b > a then tail [a..b] else  tail [a,(a-1)..b]
...
divisors n = [x | x <- [1..n], isDivisor n x , x /= n ]
...
character n = [[x] | x <- (show n)]
integer n = [(read :: String -> Int) x | x <- character n]
isRigid n = sum (integer n) == product (integer n)
